...
Write a detailed implementation plan to `.claude/adversarial/plan_v1.md`. The plan should include:
...
1. **Send plan for Codex review.** Use the Bash tool to run the codex-review script, substituting the actual iteration number for N. On iteration 2+, pass the previous review file as the 5th argument so Codex has context on what it already asked for:
   ...
   Iteration 1:
   ```
   bash ~/.claude/plugins/adversarial-review/scripts/codex-review.sh .claude/adversarial/plan_v1.md .claude/adversarial/review_v1.md plan "$(pwd)"
   ```

   Iteration 2+:
   ```
   bash ~/.claude/plugins/adversarial-review/scripts/codex-review.sh .claude/adversarial/plan_v2.md .claude/adversarial/review_v2.md plan "$(pwd)" .claude/adversarial/review_v1.md
   ```

   Always pass the previous iteration's review file (review_v{N-1}.md) as the last argument on follow-up iterations.

2. **Read the review.** Use the Read tool to read `.claude/adversarial/review_vN.md` (using the actual number).

3. **Check verdict:**
   - If the review contains `APPROVED`:
     - Copy the current plan to `.claude/adversarial/plan_final.md`
   ...
   - If the review contains `NEEDS_REVISION`:
     ...
     - Save the revised plan as `.claude/adversarial/plan_v{N+1}.md` (e.g., if N=1, save as plan_v2.md)
...
4. **If max iterations (20) reached:**
   - Save the current plan as `.claude/adversarial/plan_final.md`
...
> The adversarial plan review is complete. The plan has been saved to `.claude/adversarial/plan_final.md`.
>
